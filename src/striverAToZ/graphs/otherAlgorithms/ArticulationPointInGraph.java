...
public class ArticulationPointInGraph {
	public static void main(String[] args) {
		...
		List<Integer> res = articulationPoints(n, adj);
		for (int x : res)
			System.out.print(x + " ");
	}
...
// DFS function to identify articulation points
	private static void dfs(int node, int parent, int[] vis, int[] tin, int[] low, int[] mark,
		...
		for (int neighbor : adj.get(node)) {
			if (neighbor == parent)
				continue; // If neighbour==parent then continue
...
/// tc O(V+2E) sc O(3V)
	static List<Integer> articulationPoints(int n, List<List<Integer>> adj) {
